// Disabilitas Kabupaten Blitar
var disabilitas_blitarkab = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_blitarkab.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_blitarkab.geojson", function (data) {
	disabilitas_blitarkab.addData(data);
	map.addLayer(disabilitas_blitarkab);
});

/* Batas Desa */
map.createPane('pane_batasdesa_blitarkab');
map.getPane('pane_batasdesa_blitarkab').style.zIndex = 401;
var batasdesa_blitarkab = L.geoJson(null, {
	pane: 'pane_batasdesa_blitarkab',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_blitarkab.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_blitarkab.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_blitarkab.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_blitarkab.geojson", function (data) {
	batasdesa_blitarkab.addData(data);
	map.addLayer(batasdesa_blitarkab); //batasdesa_blitarkab ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_blitarkab');
map.getPane('pane_bataskecamatan_blitarkab').style.zIndex = 402;
var bataskecamatan_blitarkab = L.geoJson(null, {
	pane: 'pane_bataskecamatan_blitarkab',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_blitarkab.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_blitarkab.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_blitarkab.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_blitarkab.geojson", function (data) {
	bataskecamatan_blitarkab.addData(data);
	// map.addLayer(bataskecamatan_blitarkab); //batas kecamatan ditampilkan ketika halaman dipanggil
});