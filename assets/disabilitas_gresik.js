// Disabilitas Gresik
var disabilitas_gresik = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_gresik.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_gresik.geojson", function (data) {
	disabilitas_gresik.addData(data);
	map.addLayer(disabilitas_gresik);
});

/* Batas Desa */
map.createPane('pane_batasdesa_gresik');
map.getPane('pane_batasdesa_gresik').style.zIndex = 401;
var batasdesa_gresik = L.geoJson(null, {
	pane: 'pane_batasdesa_gresik',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_gresik.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_gresik.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_gresik.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_gresik.geojson", function (data) {
	batasdesa_gresik.addData(data);
	map.addLayer(batasdesa_gresik); //batasdesa_gresik ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_gresik');
map.getPane('pane_bataskecamatan_gresik').style.zIndex = 402;
var bataskecamatan_gresik = L.geoJson(null, {
	pane: 'pane_bataskecamatan_gresik',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_gresik.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_gresik.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_gresik.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_gresik.geojson", function (data) {
	bataskecamatan_gresik.addData(data);
	// map.addLayer(bataskecamatan_gresik); //batas kecamatan ditampilkan ketika halaman dipanggil
});