...
var disabilitas_lumajang = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_lumajang.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_lumajang.geojson", function (data) {
	disabilitas_lumajang.addData(data);
	map.addLayer(disabilitas_lumajang);
});

/* Batas Desa */
map.createPane('pane_batasdesa_lumajang');
map.getPane('pane_batasdesa_lumajang').style.zIndex = 401;
var batasdesa_lumajang = L.geoJson(null, {
	pane: 'pane_batasdesa_lumajang',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_lumajang.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_lumajang.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_lumajang.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_lumajang.geojson", function (data) {
	batasdesa_lumajang.addData(data);
	map.addLayer(batasdesa_lumajang); //batasdesa_lumajang ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_lumajang');
map.getPane('pane_bataskecamatan_lumajang').style.zIndex = 402;
var bataskecamatan_lumajang = L.geoJson(null, {
	pane: 'pane_bataskecamatan_lumajang',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_lumajang.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_lumajang.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_lumajang.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_lumajang.geojson", function (data) {
	bataskecamatan_lumajang.addData(data);
	// map.addLayer(bataskecamatan_lumajang); //batas kecamatan ditampilkan ketika halaman dipanggil
});