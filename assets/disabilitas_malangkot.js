// Disabilitas Kota Malang
var disabilitas_malangkot = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_malangkot.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_malangkot.geojson", function (data) {
	disabilitas_malangkot.addData(data);
	map.addLayer(disabilitas_malangkot);
});

/* Batas Desa */
map.createPane('pane_batasdesa_malangkot');
map.getPane('pane_batasdesa_malangkot').style.zIndex = 401;
var batasdesa_malangkot = L.geoJson(null, {
	pane: 'pane_batasdesa_malangkot',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_malangkot.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_malangkot.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_malangkot.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_malangkot.geojson", function (data) {
	batasdesa_malangkot.addData(data);
	map.addLayer(batasdesa_malangkot); //batasdesa_malangkot ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_malangkot');
map.getPane('pane_bataskecamatan_malangkot').style.zIndex = 402;
var bataskecamatan_malangkot = L.geoJson(null, {
	pane: 'pane_bataskecamatan_malangkot',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_malangkot.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_malangkot.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_malangkot.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_malangkot.geojson", function (data) {
	bataskecamatan_malangkot.addData(data);
	// map.addLayer(bataskecamatan_malangkot); //batas kecamatan ditampilkan ketika halaman dipanggil
});