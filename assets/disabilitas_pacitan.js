// Disabilitas Pacitan
var disabilitas_pacitan = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_pacitan.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_pacitan.geojson", function (data) {
	disabilitas_pacitan.addData(data);
	map.addLayer(disabilitas_pacitan);
});

/* Batas Desa */
map.createPane('pane_batasdesa_pacitan');
map.getPane('pane_batasdesa_pacitan').style.zIndex = 401;
var batasdesa_pacitan = L.geoJson(null, {
	pane: 'pane_batasdesa_pacitan',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_pacitan.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_pacitan.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_pacitan.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_pacitan.geojson", function (data) {
	batasdesa_pacitan.addData(data);
	map.addLayer(batasdesa_pacitan); //batasdesa_pacitan ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_pacitan');
map.getPane('pane_bataskecamatan_pacitan').style.zIndex = 402;
var bataskecamatan_pacitan = L.geoJson(null, {
	pane: 'pane_bataskecamatan_pacitan',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_pacitan.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_pacitan.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_pacitan.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_pacitan.geojson", function (data) {
	bataskecamatan_pacitan.addData(data);
	// map.addLayer(bataskecamatan_pacitan); //batas kecamatan ditampilkan ketika halaman dipanggil
});