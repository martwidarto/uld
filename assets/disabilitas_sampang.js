// Disabilitas Sampang
var disabilitas_sampang = L.geoJson(null, {
		...
		if (feature.properties) {
			...
			layer.on({
				...
				mouseover: function (e) {
					disabilitas_sampang.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC);
				}
			});
		}
	},
...
$.getJSON("data/disabilitas_sampang.geojson", function (data) {
	disabilitas_sampang.addData(data);
	map.addLayer(disabilitas_sampang);
});

/* Batas Desa */
map.createPane('pane_batasdesa_sampang');
map.getPane('pane_batasdesa_sampang').style.zIndex = 401;
var batasdesa_sampang = L.geoJson(null, {
	pane: 'pane_batasdesa_sampang',
	/* Style polygon */
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				batasdesa_sampang.bindTooltip(feature.properties.WADMKD + ", " + feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				batasdesa_sampang.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				batasdesa_sampang.bindPopup("Desa " + feature.properties.WADMKD + ", Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_desa_sampang.geojson", function (data) {
	batasdesa_sampang.addData(data);
	map.addLayer(batasdesa_sampang); //batasdesa_sampang ditampilkan ketika halaman dipanggil
});

/* Batas Kecamatan */
map.createPane('pane_bataskecamatan_sampang');
map.getPane('pane_bataskecamatan_sampang').style.zIndex = 402;
var bataskecamatan_sampang = L.geoJson(null, {
	pane: 'pane_bataskecamatan_sampang',
	/* Style polygon */
	style: function (feature) { //Fungsi style polygon
		return {
			fillColor: "white", //Warna tengah polygon
	...
	onEachFeature: function (feature, layer) {
		layer.on({
			mouseover: function (e) { //Fungsi ketika mouse berada di atas obyek
				...
				bataskecamatan_sampang.bindTooltip(feature.properties.WADMKC); //Tooltip
			},
			mouseout: function (e) { //Fungsi ketika mouse keluar dari area obyek
				bataskecamatan_sampang.resetStyle(e.target); //Mengembalikan style polygon ke style awal
				map.closePopup(); //Menutup popup
			},
			click: function (e) { //Fungsi ketika obyek di-klik
				bataskecamatan_sampang.bindPopup("Kec. " + feature.properties.WADMKC); //Popup
			}
		});
	}
});
/* memanggil data geojson polygon */
$.getJSON("data/batas_kecamatan_sampang.geojson", function (data) {
	bataskecamatan_sampang.addData(data);
	// map.addLayer(bataskecamatan_sampang); //batas kecamatan ditampilkan ketika halaman dipanggil
});